...
const buildGroqPrompt = (input: SchemaSuggestionsRequestBody): string => {
  ...
  return [
    'You are a senior database architect and canvas copilot.',
    'I will provide:',
    '1) Current schema in JSON',
    '2) Full canvas payload JSON (nodes, edges, schema metadata)',
    '3) A freeform user instruction describing what to build, change, or improve',
    '',
    'Your tasks:',
    '1) Follow the user instruction exactly when possible, using the provided schema and canvas as source of truth.',
    '2) If the instruction is broad or ambiguous, apply reasonable schema changes and explain assumptions briefly.',
    '3) Produce concise, practical textual notes describing what you changed and why.',
    '4) Produce a revised canvas payload JSON that applies those changes directly.',
    '',
    ...
    '- Update only what is necessary to satisfy the user instruction.',
    '- If the user asks for optimization suggestions, include them and apply safe improvements.',
    '- Prefer safe, realistic changes over aggressive rewrites.',
    '- If no change is needed, return the original canvas payload unchanged and explain why.',
    '',
